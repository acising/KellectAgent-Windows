...
## **WDM（windows common data model）输出格式**
   At present, WDM format conversion only applies to registry, process, thread, file, and network (Tcp) for format conversion.
   Each event will produce an edge, and the format of each edge is:
   ```json
